...
const recipes=JSON.parse(appSource.slice(start+'const recipes = '.length,end+2));
...
for(const file of modifierFiles) vm.runInContext(fs.readFileSync(path.join(root,'js',file),'utf8'),sandbox,{filename:file});
for(const key of [
...
]) sandbox.window[key]?.apply?.(recipes);
