...
const PRODUCT_PORTION_CACHE_KEY="tableBookProductPortions:v1";
...
function normalizePortionName(value){return String(value||'').toLocaleLowerCase('ru-RU').replace(/ё/g,'е').replace(/\s+/g,' ').trim();}
...
function updateProductPortionSuggestions(){const list=$('#productPortionNames'); if(!list) return; const names=[...new Set(productPortionWeights.map(row=>row.canonical_name))].sort((a,b)=>a.localeCompare(b,'ru')); list.innerHTML=names.map(name=>`<option value="${esc(name)}"></option>`).join('');}
function productUnitOptions(selected='g'){const units=[['g','г'],['piece','шт.'],['tablespoon','ст. л.'],['teaspoon','ч. л.'],['slice','ломтик'],['clove','зубчик']]; return units.map(([value,label])=>`<option value="${value}" ${value===selected?'selected':''}>${label}</option>`).join('');}
function productPortionEntry(name,unit){const key=normalizePortionName(name); if(!key||unit==='g') return null; return productPortionWeights.find(row=>row.unit_code===unit && [row.canonical_name,...row.aliases].some(value=>normalizePortionName(value)===key))||null;}
...
function productRowHtml(row={}){const unit=row.unit||'g'; const amount=row.amount??row.quantity??(unit==='g'?(row.weight??''):''); return `<div class="product-row" data-product-row><input class="input" data-prod="name" list="productPortionNames" placeholder="Например, куриное яйцо" value="${esc(row.name||'')}"><div class="product-quantity"><input class="input" data-prod="amount" type="number" min="0" step="0.1" inputmode="decimal" placeholder="кол-во" value="${esc(amount)}"><select class="select" data-prod="unit" aria-label="Единица измерения">${productUnitOptions(unit)}</select><small data-product-grams></small></div><input class="input" data-prod="kcal" type="number" min="0" step="1" placeholder="ккал" value="${esc(row.kcal??'')}"><input class="input" data-prod="protein" type="number" min="0" step="0.1" placeholder="б" value="${esc(row.protein??'')}"><input class="input" data-prod="fat" type="number" min="0" step="0.1" placeholder="ж" value="${esc(row.fat??'')}"><input class="input" data-prod="carbs" type="number" min="0" step="0.1" placeholder="у" value="${esc(row.carbs??'')}"><button class="product-remove" type="button" data-remove-product aria-label="Удалить продукт">×</button></div>`;}
function renderProductRows(rows=[{}]){const box=$('#productRows'); if(!box) return; const list=rows.length?rows:[{}]; box.innerHTML=list.map(productRowHtml).join(''); box.querySelectorAll('input,select').forEach(control=>{control.addEventListener('input',()=>{updateProductWeightHints();updateKbjuPreview();}); control.addEventListener('change',()=>{updateProductWeightHints();updateKbjuPreview();});}); box.querySelectorAll('[data-remove-product]').forEach(btn=>btn.onclick=()=>{const rows=getProductRows({includeEmpty:true}); const idx=[...box.querySelectorAll('[data-remove-product]')].indexOf(btn); rows.splice(idx,1); renderProductRows(rows.length?rows:[{}]); updateKbjuPreview();}); updateProductWeightHints();}
...
function getProductRows({includeEmpty=false}={}){return $$('#productRows [data-product-row]').map(row=>{const get=n=>row.querySelector(`[data-prod="${n}"]`)?.value?.trim()||''; const name=get('name'),amount=Number(get('amount'))||0,unit=get('unit')||'g'; const resolved=productWeightFor(name,amount,unit); const item={name,amount,unit,weight:resolved.weight,gramsPerUnit:resolved.gramsPerUnit,kcal:Number(get('kcal'))||0,protein:Number(get('protein'))||0,fat:Number(get('fat'))||0,carbs:Number(get('carbs'))||0}; return item;}).filter(item=>includeEmpty || item.name || item.amount || item.kcal || item.protein || item.fat || item.carbs);}
function updateProductWeightHints(){const rows=$$('#productRows [data-product-row]'); rows.forEach(row=>{const name=row.querySelector('[data-prod="name"]')?.value||'',amount=Number(row.querySelector('[data-prod="amount"]')?.value)||0,unit=row.querySelector('[data-prod="unit"]')?.value||'g',hint=row.querySelector('[data-product-grams]'); if(!hint) return; const resolved=productWeightFor(name,amount,unit); if(!amount){hint.textContent='';hint.classList.remove('missing');return;} if(unit==='g'){hint.textContent=`${fmt(amount)} г`;hint.classList.remove('missing');return;} if(resolved.weight>0){hint.textContent=`≈ ${fmt(resolved.weight)} г${resolved.entry?.note?` · ${resolved.entry.note}`:''}`;hint.classList.remove('missing');}else{hint.textContent='Нет такой порции в справочнике — выберите граммы';hint.classList.add('missing');}});}
function calcFromProducts(){const products=getProductRows(); const used=products.filter(p=>p.name && p.weight>0); if(!used.length) return null; const inputWeight=used.reduce((s,p)=>s+p.weight,0); if(inputWeight<=0) return null; const total=used.reduce((acc,p)=>{const k=p.weight/100; acc.kcal+=p.kcal*k; acc.protein+=p.protein*k; acc.fat+=p.fat*k; acc.carbs+=p.carbs*k; return acc;},{kcal:0,protein:0,fat:0,carbs:0}); return {inputWeight,total,products:used};}
...
function updateKbjuPreview(){const el=$('#myKbjuPreview'); if(!el) return; const data=getCustomNutrition(); if(data.requiresWeight){const total=data.total?nutritionLine('Во всём блюде',data.total):''; const input=data.inputWeight?` Масса продуктов до приготовления: ${fmt(data.inputWeight)} г.`:''; el.innerHTML=`<div class="kbju-source">Расчёт почти готов</div>${total}<span class="kbju-muted">Укажите вес готового блюда, чтобы получить КБЖУ на 100 г и на порцию.${input}</span>`; return;} if(!data.nutrition){el.innerHTML='<span class="kbju-muted">КБЖУ рассчитается после ввода продуктов или итоговых значений на 100 г.</span>'; return;} const src=data.source==='products'?`По продуктам${data.inputWeight?` · до приготовления ${fmt(data.inputWeight)} г`:''}`:'По готовым значениям на 100 г'; el.innerHTML=`<div class="kbju-source">${src} · готовое блюдо ${fmt(data.weight)} г</div><div class="kbju-summary-grid">${nutritionLine('В готовом блюде',data.total)}${nutritionLine('На 100 г',data.nutrition100)}${nutritionLine('На порцию',data.nutrition)}</div>`;}
function saveCustomRecipe(){
  ...
  if(!ingredients.length){alert('Добавьте ингредиенты рецепта. Продукты из КБЖУ-калькулятора используются только для расчёта и не переносятся автоматически.'); return;}
